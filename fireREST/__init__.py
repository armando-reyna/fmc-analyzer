...
class FireREST(object):
    ...
    def get_audit_records(self, object_id):
        request = '/audit/auditrecords/{0}'.format(object_id)
        url = self._url('config', request)
        return self._get(url)
    ...
    def get_acps(self):
        request = '/policy/accesspolicies'.format()
        url = self._url('config', request)
        return self._get(url)

    def get_acp(self, object_id):
        request = '/policy/accesspolicies/{0}'.format(object_id)
        url = self._url('config', request)
        return self._get(url)
    ...
    def get_networkgroup_id_by_name(self, protocol, networkgroup_name):
        rest_objs = self.get_objects(protocol, False)
        for rest_obj in rest_objs:
            networkgroups_obj = rest_obj.json()
            if 'items' in networkgroups_obj:
                networkgroups = networkgroups_obj['items']
                for networkgroup in networkgroups:
                    if networkgroup['name'] == networkgroup_name:
                        return networkgroup['id']
            else:
                print('protocol: {}'.format(protocol))
                self.print_json(rest_obj)
        return None

    def get_immediate_parent(self, networkgroup_parent, host):

        is_network = False
        networkgroup_id = self.get_networkgroup_id_by_name('networkgroups', networkgroup_parent)

        if networkgroup_id is None:
            networkgroup_id = self.get_networkgroup_id_by_name('portobjectgroups', networkgroup_parent)
        else:
            is_network = True

        if networkgroup_id is None:
            print('networkgroup: {} not found'.format(networkgroup_parent))
            return networkgroup_parent

        print('networkgroup: {}  id: {}  host: {}'.format(networkgroup_parent, networkgroup_id, host))

        if is_network :
            rest_objs = self.get_object('networkgroups', networkgroup_id)
        else:
            rest_objs = self.get_object('portobjectgroups', networkgroup_id)

        networkgroups_dict = dict()

        for rest_obj in rest_objs:
            networkgroup_obj = rest_obj.json()
            if 'objects' in networkgroup_obj:
                networkgroups = networkgroup_obj['objects']
                for networkgroup_child in networkgroups:
                    # print('type: {} name: {}'.format(networkgroup_child['type'], networkgroup_child['name']))
                    if networkgroup_child['type'] == 'Host':
                        if networkgroup_child['name'] == host:
                            return networkgroup_parent
                    elif networkgroup_child['type'] == 'NetworkGroup':
                        child_id = networkgroup_child['id']
                        rest_objs2 = self.get_object('networkgroups', child_id)
                        for rest_obj2 in rest_objs2:
                            networkgroup_child_obj = rest_obj2.json()
                            if 'id' in networkgroup_child_obj:
                                obj_id = networkgroup_child_obj['id']
                                if 'literals' in networkgroup_child_obj:
                                    networkgroup_new_child_obj = {"id": obj_id,
                                                                  "name": networkgroup_child_obj['name'],
                                                                  "type": networkgroup_child_obj['type'],
                                                                  "literals": networkgroup_child_obj['literals']}
                                    networkgroups_dict[obj_id] = networkgroup_new_child_obj
                                if 'objects' in networkgroup_child_obj:
                                    networkgroup_new_child_obj = {"id": obj_id,
                                                                  "name": networkgroup_child_obj['name'],
                                                                  "type": networkgroup_child_obj['type'],
                                                                  "objects": networkgroup_child_obj['objects']}
                                    networkgroups_dict[obj_id] = networkgroup_new_child_obj
                                else:
                                    self.print_json(rest_obj2)
            else:
                self.print_json(rest_obj)

        for key in networkgroups_dict:
            networkgroup = networkgroups_dict[key]['name']
            # print('Analyzing {}'.format(networkgroup))
            if 'literals' in networkgroups_dict[key]:
                literals = networkgroups_dict[key]['literals']
                for literal in literals:
                    if literal['type'] == 'Host':
                        if literal['value'] == host:
                            return networkgroup
            elif 'objects' in networkgroups_dict[key]:
                objects = networkgroups_dict[key]['objects']
